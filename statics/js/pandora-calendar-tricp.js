(function(global, $, pandora, undefined) {
    ...
    Calendar.prototype = {
            ...
            dates.forEach(function(d) {
                d.forEach(function(d) {
                    day += that.replaceWith(options.template.day, {
                        ...
                        className: d === "" ?'class="nodate"':'class="' + that.getClass(d) + '"'
                    });
                });
            ...
            if (date === "") {
                ...
                if (this._isRange) {
                    ...
                    if (index > 0) {
                        ...
                        d2 = new Date(val1[0], val1[1] === "12" ? "11" : val1[1] - 1, val1[2]);
                    } else
                        d2 = new Date();
                }
        ...
        changeMonth: function(op) {
            ...
            if (options.control) {
                //修改控件
                //options.showPrev = options.date.getFullYear() > cacheDate.getFullYear() ? true : options.date.getMonth() === cacheDate.getMonth() ? false : true;
        ...
        triggerBlur: function() {
            ...
            $(document).click(function(e) {
                var target = $(e.target);
                if (!target.hasClass(options.triggerClass) && !target.parents().hasClass(options.warpClass) && !target.hasClass(options.classNames.monthPrev) && !target.hasClass(options.classNames.monthNext)) {
                    that.warp.detach();
                }
            });
        }
    ...
    var smallTemplate = {
        ...
        calBody: '<div class="calbox">' + '<i class="monthbg">{{month}}</i>' + '<table cellspacing="0" cellpadding="0" border="0" class="caltable">' + '<thead>' + '<tr>' + '<th class="sun">日</th>' + '<th class="mon">一</th>' + '<th class="tue">二</th>' + '<th class="wed">三</th>' + '<th class="thu">四</th>' + '<th class="fri">五</th>' + '<th class="sat">六</th>' + '</tr>' + '</thead>' + '<tbody>' + '{{date}}' + '</tbody>' + '</table>' + '</div>',
        weekWarp: '<tr>{{week}}</tr>',
        day: '<td {{week}} {{dateMap}} >' + '<div {{className}}>{{day}}</div>' + '</td>'
    };
    var bigTemplate = {
        ...
        calBody: '<div class="calbox">' + '<i class="monthbg">{{month}}</i>' + '<table cellspacing="0" cellpadding="0" border="0" class="caltable">' + '<thead>' + '<tr>' + '<th class="sun">日</th>' + '<th class="mon">一</th>' + '<th class="tue">二</th>' + '<th class="wed">三</th>' + '<th class="thu">四</th>' + '<th class="fri">五</th>' + '<th class="sat">六</th>' + '</tr>' + '</thead>' + '<tbody>' + '{{date}}' + '</tbody>' + '</table>' + '</div>',
        weekWarp: '<tr>{{week}}</tr>',
        day: '<td {{week}} {{dateMap}} >' + '<div {{className}}>' + '<span class="calday">{{day}}</span>' + '<span class="calinfo"></span>' + '<span class="calprice"></span>' + '<span class="calactive"></span>' + '</div>' + '</td>'
    };
